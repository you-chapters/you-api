...
def clear_caches():
    ...
    deps._embedding_client.cache_clear()
    deps._vector_repository.cache_clear()
    deps._llm_client.cache_clear()
    deps._narrative_repository.cache_clear()
    yield
    deps._repository.cache_clear()
    deps._embedding_client.cache_clear()
    deps._vector_repository.cache_clear()
...
def test_repository_returns_dynamodb_when_configured(monkeypatch) -> None:
    ...
    monkeypatch.setenv("ENTRIES_TABLE_NAME", "test-table")
...
def test_embedding_client_returns_in_memory_by_default(monkeypatch) -> None:
    monkeypatch.delenv("EMBEDDING_TYPE", raising=False)

    assert isinstance(deps._embedding_client(), InMemoryEmbeddingClient)


def test_embedding_client_returns_openai_when_configured(monkeypatch) -> None:
    ...
    with patch("app.embedding.openai_embedding_client.OpenAI"):
        ...
        assert isinstance(deps._embedding_client(), OpenAIEmbeddingClient)
...
def test_llm_client_returns_in_memory_by_default(monkeypatch) -> None:
    monkeypatch.delenv("LLM_TYPE", raising=False)

    assert isinstance(deps._llm_client(), InMemoryLLMClient)


def test_llm_client_returns_openai_when_configured(monkeypatch) -> None:
    monkeypatch.setenv("LLM_TYPE", "openai")
    monkeypatch.setenv("OPENAI_API_KEY", "test-key")

    with patch("app.llm.openai_llm_client.OpenAI"):
        from app.llm.openai_llm_client import OpenAILLMClient
        assert isinstance(deps._llm_client(), OpenAILLMClient)


def test_narrative_repository_returns_in_memory_by_default(monkeypatch) -> None:
    monkeypatch.delenv("NARRATIVES_TABLE_NAME", raising=False)

    assert isinstance(deps._narrative_repository(), InMemoryNarrativeRepository)


def test_narrative_repository_returns_dynamodb_when_configured(monkeypatch) -> None:
    monkeypatch.setenv("NARRATIVES_TABLE_NAME", "narratives")

    with patch("app.repositories.narrative_repository.boto3.resource"):
...
def test_get_narrative_service_returns_narrative_service(monkeypatch) -> None:
    ...
    assert isinstance(get_narrative_service(), NarrativeService)
